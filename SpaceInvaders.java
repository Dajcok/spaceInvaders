...
class targetWindow
{
	private Vector2f enemies1Pos = new Vector2f(0, 0);
	private int mark = 1;
	...
	targetWindow(int W, int H, int frameLimit)
	{
		...
		while(window.isOpen())
		{
			...
			enemyMovement(enemies1);
			enemyMovement(enemies2);
			enemyMovement(enemies3);
		...
		for(int i = 0; i < enemies.size(); i++)
		{
			if(enemies.elementAt(i).getSpritePos().x > 1870)
			{
				...
				for(int j = 0; j < enemies.size() ; j++)
					enemies.elementAt(j).updatePosition(new Vector2f(enemies.elementAt(j).getSpritePos().x - 0.5f, enemies.elementAt(j).getSpritePos().y + 20));
				this.changeDir = true;
	...
	void enemyMovement_direction(int i, Vector<texturesLoader> enemies)
	{
		if(this.changeDir)
		{
			enemies.elementAt(i).updatePosition(new Vector2f (enemies.elementAt(i).getSpritePos().x - 0.5f, enemies.elementAt(i).getSpritePos().y));
		}
